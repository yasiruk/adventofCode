...
private const val sampleData = """light red bags contain 1 bright white bag, 2 muted yellow bags.
...
private const val sampleData2 = """shiny gold bags contain 2 dark red bags.
...
private const val testData = """pale chartreuse bags contain 3 faded orange bags.
...
class HandyHaversacksTest : StringSpec({
    val h = HandyHaversacks()

    "rule parser" {
        h.getRule("light red bags contain 1 bright white bag, 2 muted yellow bags.") shouldBe
                ("light red" to setOf(
                    BagSet("1 bright white"),
                    BagSet("2 muted yellow")
                ))
    }

    "test" {
        val t = h.run(sampleData)
        t.shouldHaveSize(4)
    }

    "test case" {
        h.run(testData).shouldHaveSize(296)
    }

    "sample case sum" {
        h.totalBags(sampleData) shouldBe 32
    }

    "sample case sum 2" {
        h.totalBags(sampleData2) shouldBe 126
    }

    "test case sum"  {
        h.totalBags(testData) shouldBe 125
    }
})
