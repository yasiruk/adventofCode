...
private const val sampleData = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
private const val testData1 = """ecl:amb
pid:690616023
...
private const val invalidSampleData = """eyr:1972 cid:100
...
private const val validSampleData = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
class PassportProcessingTest : StringSpec({
    val p = PassportProcessing()
    "passport strings" {
        val pp = p.getPassportStrings(sampleData)
        pp.shouldHaveSize(4)

        sampleData.splitAnswers().shouldHaveSize(4)
    }

    "sample test" {
        val pp = p.run(sampleData)
        pp.shouldHaveSize(2)
    }

    "test data 1" {
        val pp = p.run(testData1)
//        pp.shouldHaveSize(170)
        pp.shouldHaveSize(103)
    }

    "height test" {
        p.validateHeight("167cm").shouldBeTrue()
        p.validateHeight("60in").shouldBeTrue()
        p.validateHeight("160in").shouldBeFalse()
        p.validateHeight("160").shouldBeFalse()
        p.validateHeight("58in").shouldBeFalse()
        p.validateHeight("200cm").shouldBeFalse()
        p.validateHeight("149cm").shouldBeFalse()
    }

    "invalid sample data" {
        p.run(invalidSampleData).shouldHaveSize(0)
    }

    "valid sample data" {
        p.run(validSampleData).shouldHaveSize(4)
    }
})
